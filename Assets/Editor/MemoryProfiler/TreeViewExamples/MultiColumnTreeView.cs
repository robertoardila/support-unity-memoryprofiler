...
namespace UnityEditor.TreeViewExamples
{
	...
	internal class MultiColumnTreeView : TreeViewWithTreeModel<MyTreeElement>
	{
		...
		public int Compare (TreeViewItem x, TreeViewItem y, SortOption sortOption)
		{
			...
			MyTreeElement myTreeElementX = ((TreeViewItem<MyTreeElement>)x).data;
			MyTreeElement myTreeElementY = ((TreeViewItem<MyTreeElement>)y).data;
			switch (sortOption)
			{
				case SortOption.Name:
					return EditorUtility.NaturalCompare(myTreeElementX.name, myTreeElementY.name);
				case SortOption.Value1:
					return myTreeElementX.floatValue1.CompareTo(myTreeElementY.floatValue1);
				case SortOption.Value2:
					return myTreeElementX.floatValue2.CompareTo(myTreeElementY.floatValue2);
				case SortOption.Value3:
					return myTreeElementX.floatValue3.CompareTo(myTreeElementY.floatValue3);
			}
			return 0;
		}
				...
				case MyColumns.Value3:
					{
						if (showControls)
						{
							cellRect.xMin += 20; // When showing controls make some extra spacing
		...
		public static MultiColumnHeaderState CreateDefaultMultiColumnHeaderState(float treeViewWidth)
		{
			var columns = new[] 
			{
				...
				new MultiColumnHeaderState.Column 
				{
					headerContent = new GUIContent("Total", "In sed porta ante. Nunc et nulla mi."),
				...
				new MultiColumnHeaderState.Column 
				{
					headerContent = new GUIContent("Time ms", "Maecenas congue non tortor eget vulputate."),
					headerTextAlignment = TextAlignment.Right,
					sortedAscending = true,
					sortingArrowAlignment = TextAlignment.Left,
					width = 60,
				...
				new MultiColumnHeaderState.Column 
				{
					headerContent = new GUIContent("Self ms", "Nam at tellus ultricies ligula vehicula ornare sit amet quis metus."),
					headerTextAlignment = TextAlignment.Right,
					sortedAscending = true,
					sortingArrowAlignment = TextAlignment.Left,
					width = 60,
